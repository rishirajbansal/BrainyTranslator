...
then
    ...
    # TODO: WEB service has 2 ports assigned and thats why following command fdoes not work, need to check upon, 
    # for now assigning hard coded port
    #web_port=$(docker inspect --format='{{range $p, $conf := .NetworkSettings.Ports}}{{(index $conf 0).HostPort}} {{end}}' ${WEB_CONTAINER_NAME})
    ...
    #echo "DB Service IP : $(docker inspect --format='{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' ${DB_CONTAINER_NAME})"
    #echo "API Service IP : $(docker inspect --format='{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' ${API_CONTAINER_NAME})"
    #echo "WEB Service IP : $(docker inspect --format='{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' ${WEB_CONTAINER_NAME})"
